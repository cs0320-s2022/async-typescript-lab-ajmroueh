...
const listElem = document.getElementById("suggestions") as HTMLUListElement;
...
function postAndUpdate(): void {
  ...
  //  HINT: use .innerHTML
  const sugList = document.getElementById("suggestions") as HTMLUListElement;
  sugList.innerHTML = "";
  ...
  const res: Promise<Response> = fetch("http://localhost:4567/matches", {
    method: "POST",
    headers: {
  ...
  res.then(response => response.json()).then((data: Matches) => updateSuggestions(data.matches));
}
